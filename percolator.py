    ...
    def ChooseVertexToRemove(graph, player):
        ...
        for x in graph.V:
            if x.color == -1:  
                uncolored.append(x)
        for i in uncolored:
            numVerticesOurs = len(graph.V) - PercolationPlayer.numVerticesNotOurs(graph, i, player) 
            num_color[i] = PercolationPlayer.numVerticesNotOurs(graph, i, player) - numVerticesOurs
        best_vertex_pair = sorted(num_color.items(), key=lambda x: x[1], reverse = True)[0]
        return best_vertex_pair[0]